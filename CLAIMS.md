...
| claude-opus-4.6-ux | login-ux-cleanup-v1 — drop owner@catalyst placeholder, simpler brand, back button at top; then notifications+todos plumbing for password_reset_requests so admins get a fresh signal when a reset lands (no overlap with other active rows) | templates/login.html, app.py (notify_admins_on_pw_reset helper only — no other app.py changes) | 2026-04-15 20:10 CEST |
| claude-opus-4.6-docs | sanity-wave-reference-doc — write docs/SANITY_WAVE.md as living reference of the 13 pre-push gates (what each checks, common WARN patterns, fix recipes, how to add a new gate) | docs/SANITY_WAVE.md (new) | 2026-04-15 15:45 CEST |
