#!/bin/bash
# Install the PRISM pre-receive gate into the laptop's central bare.
#
...
BARE="${PRISM_BARE:-$HOME/.claude/git-server/lab-scheduler.git}"
HOOK_SRC="$SCRIPT_DIR/pre-receive"
HOOK_DST="$BARE/hooks/pre-receive"

if [ ! -d "$BARE" ]; then
  echo "error: laptop bare not found at $BARE" >&2
  echo "set PRISM_BARE=<path> or ensure the bare exists." >&2
  exit 1
fi
if [ ! -f "$HOOK_SRC" ]; then
